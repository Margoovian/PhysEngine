...
ApplicationHarness::ApplicationHarness(Application* appInit) : app(appInit)
{
	...
	window = glfwCreateWindow(1280, 720, "PhysEngine", nullptr, nullptr);
	...
	GLFWimage images[1]; // Creates image buffer
...
void ApplicationHarness::OnKey(int key, char action)
{
	if (action == 'a') app->OnKeyPressed(key);
	else if (action == 'b') app->OnKeyReleased(key);
	else if (action == 'c') app->OnKeyHold(key);
}
